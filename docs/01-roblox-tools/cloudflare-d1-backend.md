...
- `functions/api/reports/[id]/status.js`: admin review status updates for player reports.
...
- Admin mode is opened with `?admin=1#signals`. It shows player reports, lets the owner copy report details, and mark reports as `pending` or `reviewed`.
